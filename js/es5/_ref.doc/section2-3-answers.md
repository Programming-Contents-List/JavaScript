...
## 보충: Layer와 Slice 개념 상세 설명

### Layer(레이어)
- **수평적 구조**: 애플리케이션의 계층을 나타냄
- **기술적 책임**: 각 레이어는 특정 수준의 기술적 책임을 가짐
- **의존성 방향**: 상위 레이어는 하위 레이어에 의존 (단방향)

```md
View(page)     → 사용자가 보는 화면
    ↓ 사용
Hook           → 재사용 가능한 로직
    ↓ 사용
Service        → 외부 API 통신
...
Manager        → 비즈니스 로직 관리
    ↓ 사용
Object(type)   → 데이터 구조 정의
```

### Slice(슬라이스)
- **수직적 구조**: 비즈니스 도메인 또는 기능 단위를 나타냄
- **도메인 책임**: 각 슬라이스는 독립적인 비즈니스 기능을 담당
- **격리성**: 같은 레벨의 슬라이스끼리는 서로 의존하지 않음

```md
features/
  ├── authentication/    # 인증 관련 기능 (슬라이스)
  │   ├── view/
  │   ├── hook/
  │   ├── service/
  │   └── manager/
...
### Layer와 Slice의 교차점
```md
              authentication   cart   payment  (Slice - 수직)
                    │           │       │
View           ─────┼───────────┼───────┼──────
Hook           ─────┼───────────┼───────┼──────  (Layer - 수평)
Service        ─────┼───────────┼───────┼──────
...
**핵심 차이점:**
- **Layer**: "무엇을 하는가" (기술적 관점)
- **Slice**: "무엇을 위한 것인가" (비즈니스 관점)